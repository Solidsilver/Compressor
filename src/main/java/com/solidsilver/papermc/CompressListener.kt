...
class CompressListener(private val plugin: Compressor): Listener {
    ...
    fun asCraft(event: PrepareItemCraftEvent) {
        val items = event.inventory.matrix
        var numCompItems = 0
        var cmpLvl = 0
        var itemType: Material? = null
        for (item in items) {
            if (item?.itemMeta?.persistentDataContainer?.has(plugin.getCompLevelKey()) == true) {
                val itmLvl = item.itemMeta.persistentDataContainer.get(plugin.getCompLevelKey(), PersistentDataType.INTEGER) ?: 0
                if (itemType == null) {
                    itemType = item.type
                    cmpLvl = itmLvl
